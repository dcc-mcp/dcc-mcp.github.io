...
export const dccIntegrations: DccIntegration[] = [
  ...
  },
  {
    ...
    dccType: 'shogun',
    summaryEn: 'use 48 typed official-SDK tools to inspect motion-capture scenes, attributes, bounded channel data, optical cameras, and files, control timeline state, and run capability-gated processing',
    summaryZh: '通过 48 个基于官方 SDK 的类型化工具检查动捕场景、属性、有界 Channel 数据、光学相机与文件，控制时间线状态，并执行带能力门控的处理',
    tasksEn: ['inspect subjects, markers, skeletons, trajectories, hierarchy, transforms, attribute and channel names, bounded samples and gaps, optical cameras, and selection', 'inspect or change explicit play and animation ranges, key-derived selections, and playback when the host exposes Timeline', 'inspect or update an allowlisted processing setting and run ROM labeling, calibration, QuickPost, or another bounded processing step only after the host confirms the official capability'],
    tasksZh: ['检查角色、Marker、骨架、轨迹、层级、变换、属性与 Channel 名称、有界样本与缺口、光学相机和选择集', '在宿主公开 Timeline 能力时检查或修改明确的播放/动画范围、由关键帧生成的选择集与播放状态', '仅在宿主确认官方能力后检查或更新白名单内的处理设置，并执行 ROM 标记、校准、QuickPost 或其他有界处理'],
  },
  {
    slug: 'sketchup',
    name: 'SketchUp',
