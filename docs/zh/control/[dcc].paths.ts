  ...
  paths: () => dccIntegrations.map((integration) => ({
    params: {
      ...
      description: integration.slug === 'comfyui'
        ...
        : integration.dccType || integration.coreApplicationRoute ? `通过 DCC-MCP ${integration.name} MCP 适配器和 dcc-mcp-cli，使用类型化工具、安全发现与结果验证控制 ${integration.name}。` : `通过 DCC-MCP 的类型化工具、安全发现与结果验证，让 AI Agent 控制 ${integration.name}。`,
    },
